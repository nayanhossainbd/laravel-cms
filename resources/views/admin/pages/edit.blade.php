...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pages.update", [$page->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('template') ? 'is-invalid' : '' }}" name="template" id="template" required>
                    ...
                    @foreach(App\Models\Page::TEMPLATE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('template', $page->template) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div id="body_content_new" class="form-group">
                <label for="body_content">{{ trans('cruds.page.fields.body_content') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('body_content') ? 'is-invalid' : '' }}" name="body_content" id="body_content">{!! old('body_content', $page->body_content) !!}</textarea>
