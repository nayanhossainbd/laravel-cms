...
class GalleryController extends Controller
{
    ...
    public function store(StoreGalleryRequest $request)
    {
        ...
        foreach ($request->input('gallery_img', []) as $file) {
            $gallery->addMedia(storage_path('media/uploads/' . $file))->toMediaCollection('gallery_img');
        }
    ...
    public function update(UpdateGalleryRequest $request, Gallery $gallery)
    {
        ...
        if (count($gallery->gallery_img) > 0) {
            foreach ($gallery->gallery_img as $media) {
                if (!in_array($media->file_name, $request->input('gallery_img', []))) {
                    $media->delete();
                }
            }
        }

        $media = $gallery->gallery_img->pluck('file_name')->toArray();
